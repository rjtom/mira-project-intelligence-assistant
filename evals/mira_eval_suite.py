import requests, json, time, csv, os, re, glob
...
def extract_ground_truth(filepath):
    ...
    truth = {"project_name": "", "timeline": "", "status": "",
             ...
             "governance": [], "lessons": [], "full_content": content}
...
def get_ground_truth_text(truth, question_type):
    sections = {
        "timeline": (f"Timeline: {truth.get('timeline','N/A')}
"
...
" + "
".join(f"- {o}" for o in truth.get("objectives",[])),
        "governance": "Governance:
" + "
".join(f"- {g}" for g in truth.get("governance",[])),
        "lessons": "Lessons:
" + "
".join(truth.get("lessons",["No lessons found"])),
        "resources": "Resources:
" + "
".join(f"- {r}" for r in truth.get("resources",[])),
        "risks": "Risk data from Google Sheets",
    }
    return sections.get(question_type, truth.get("full_content",""))[:2000]
...
def llm_judge(question, ground_truth, response, question_type):
    ...
    try:
        ...
        prompt = f"""You are an expert evaluator for MIRA AI project intelligence system.
Question type: {question_type}
Question: {question}
Ground truth: {ground_truth}
MIRA response: {response}

Score each 0-10. Return ONLY valid JSON:
{{"factual_accuracy":<0-10>,"groundedness":<0-10>,"completeness":<0-10>,"hallucination_free":<0-10>,"relevance":<0-10>,"overall":<average>,"issues":["issue1"],"reasoning":"brief explanation"}}"""
...
def run_eval_suite():
    ...
    for i, project in enumerate(projects):
        ...
        for q_type, template in all_templates:
            ...
            if ENABLE_LLM_JUDGE and result["success"] and result["response"]:
                ...
                judge_result = llm_judge(question, gt_text, result["response"], q_type)
                if judge_result.get("overall", 0) > 0:
                    judge_scores.append(judge_result["overall"])
            if content_ok: passed += 1
            else: failed += 1
    ...
    for q_type, _ in all_templates:
        ...
        scores = [r["judge_overall"] for r in tr if isinstance(r.get("judge_overall"), (int, float))]
